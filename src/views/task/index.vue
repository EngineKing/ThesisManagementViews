    ...
    >
      ...
      <el-table-column label="操作" align="center" min-width="180" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          ...
          <el-button type="gray" size="mini" @click="handleUpdate(row)">上传附件</el-button>
          <el-button type="success" size="mini" @click="handleUpdate(row)">任务通知</el-button>
          <el-button type="info" size="mini" style="margin-top:8px;" @click="handleUpdate(row)">导入名单</el-button>
          <el-button type="danger" size="mini" style="margin-top:8px;" @click="handleUpdate(row)">查看结果</el-button>
        </template>
      </el-table-column>
    </el-table>
